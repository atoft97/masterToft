...
def write_to_csv(new_data, fileName):
    with open(fileName, 'a') as f_object:
        writer_object = DictWriter(f_object, fieldnames=['time', 'latitude', 'longitude'])
...
for bag_file in [bag_files[-1]]:
    ...
    if (os.path.exists(csvPath)):
        os.remove(csvPath)
    write_to_csv({'time': 'time', 'latitude': 'latitude', 'longitude': 'longitude'}, csvPath)
    ...
    for myTopic in ['/ugv_sensors/navp_ros/nav_sat_fix']:
        ...
        for topic, msg, t in tqdm(bag.read_messages(topics=myTopic)):
            ...
            #print(msg)
            #print(t)
            timePOS = {'time': t,'latitude': msg.latitude, 'longitude': msg.longitude}
