...
def print_projection_plt(points, color, image, lidarImageColor, lidarLabelColor):
    ...
    for i in range(points.shape[1]):
        ...
        cv2.circle(image3Channels, (np.int32(points[0][i]),np.int32(points[1][i])),2, pixelColor,-1)

        pixelColor = (int(lidarImageColor[i][3]), int(lidarImageColor[i][3]),int(lidarImageColor[i][3]))
        cv2.circle(image1Channel, (np.int32(points[0][i]),np.int32(points[1][i])),2, pixelColor,-1)

        pixelColor = (int(lidarLabelColor[i][0]), int(lidarLabelColor[i][1]),int(lidarLabelColor[i][2]))
        cv2.circle(imageLabel, (np.int32(points[0][i]),np.int32(points[1][i])),2, pixelColor,-1)
...
def lidarRGBcombine(lidarPath, rgbPath, lidarImagePath, lidarNumpyPath, lidarLabelPath):
    ...
    cv2.imwrite("projektertLidar2.png", visualised)
