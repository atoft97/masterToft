...
#bag_files_folder = "/home/potetsos/lagrinatorn/master/bagfiler/orginal"
...
output_dir = "../data/rgbFileName2"

bagFoldersPath = "/home/potetsos/lagrinatorn/master/bagfiler"
bagFolders = os.listdir(bagFoldersPath)
#print(bagFolders)
print(bagFolders[0])

totalTimeDifferance = 0

for bagFolder in [bagFolders[0]]:
    bag_files_folder = f"{bagFoldersPath}/{bagFolder}"
    bag_files = listdir(bag_files_folder)
    print(bag_files)
    #bag_files = ["stand_still_short.bag"]
    for bag_file in [bag_files[6]]:
        print("\n")
        ...
        for imageToptic in ['/ugv_sensors/camera/color/image']:
        #for imageToptic in ['/ugv_sensors/lidar/image/signal_image']:
            ...
            for topic, msg, t in tqdm(bag.read_messages(topics=imageToptic)): 
                ...
                #if (bag_file in bag2Name.keys()):
                #    timePOS = {'time': f"{msg.header.stamp.secs}.{msg.header.stamp.nsecs}",'filename': bag2Name[bag_file] + "_frame" + str(count).zfill(5) +".png"}
                #else:
                #timePOS = {'time': f"{msg.header.stamp.secs}.{msg.header.stamp.nsecs}",'filename': bag_file[:-4] + "_" + str(count).zfill(5) +".png"}
                ...
                recordedTime = float(f"{msg.header.stamp.secs}.{msg.header.stamp.nsecs}")
                print(recordedTime)
                
                writtenTime = float(str(t)) / 10**9
                print("written", writtenTime)


                if (writtenTime > recordedTime):
                    print("time differance", writtenTime-recordedTime)
                    totalTimeDifferance += (writtenTime-recordedTime)
                    print("average time differance", (totalTimeDifferance/(count+1)))
