...
#    os.remove(csvPath)
    ...
    def getDirectionOfImage(self,indexImage, colorImage, fileName, rangeImage, rgb_img, drivableColor, useDepth, lat, long, heading): 
        ...
        x_coord = math.sin(rad)*50  + longitude
        y_coord = math.cos(rad)*50 + latitude

        rad_actual = math.radians(direction)
        x_coord_actual = math.sin(rad_actual)*50  + longitude
        y_coord_actual = math.cos(rad_actual)*50 + latitude
        '''
        ...
        x_coord_best = math.sin(rad_best)*50  + longitude
        y_coord_best = math.cos(rad_best)*50 + latitude
        ...
        if (useDepth):
            ...
            partImageVisual[rangeImage<75] = 0
