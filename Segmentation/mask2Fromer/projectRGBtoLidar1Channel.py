...
def print_projection_plt(points, color, image, lidarImageColor, lidarLabelColor, resLidarIndex):
    ...
    image4Channel = np.zeros((image.shape[0], image.shape[1], 3+1))
    ...
    #expanded = expand_labels(image, distance=3)
        #break
    return(image,image4Channel[:,:,0:3], image4Channel, imageLabel, pointsColors)
...
def lidarRGBcombine(lidarPath, rgbPath, lidarImagePath, lidarNumpyPath, lidarLabelPath):
    ...
    visualised, lidarImage, lidarNumpy, lidarLabels, pointsColors = print_projection_plt(points=imgpoints, color=c_, image=image, lidarImageColor=lidarImageColor, lidarLabelColor=lidarLabelColor, resLidarIndex=resLidarIndex)
    ...
    #print("points shape", pointsColors.shape)
    #print(pointsColors[100:200,:])
    #print("lidar form", lidarNumpy.shape)
    

    colorImageBack = np.zeros((64*2048))
    ...
    #    numpy4channelColor = lidarNumpyImageSmaller_staggeredFloat[index]
        ...
        #print(color)
    colorImageBackReshaped = colorImageBack.reshape(64, 2048)
    ...
    #cv2.imwrite("pointColor.png", colorImageBackDestaggered)
    #cv2.imwrite("lidarBack.png", lidarNumpyBackReshaped[:,:,0:3])
    ...
    return(colorImageBackDestaggered, visualised)
