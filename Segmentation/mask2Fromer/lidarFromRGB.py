...
for fileName in tqdm(files):
    ...
    lidarSegmentedClassFullPath = f"{outputFolder}/lidarPredClass/{fileName}"
    
    lidarSegmentedClass = cv2.imread(lidarSegmentedClassFullPath, -1)
    ...
    startTime = time.time()
    rgbLabledLidar, visualised = lidarRGBcombine(lidarCloudFullPath,predClassNumpy , lidarImageFullPath, lidarNumpyFullPath , lidarLabelFullPath)
    diffTime = time.time() - startTime
    print("Project Tid:", diffTime)
    totalTime += diffTime
    totalTimeCounter += 1
    print("Project Tid avg:", totalTime/totalTimeCounter)
    ...
    gt = lidarLabelID
    #gt = gtImage
    predFromData = rgbLabledLidarTaller

    #gt[:,:] = 4
    #pred[:,:] = 4

    pred = lidarSegmentedClass

    print("pred", pred.shape)
    print("gt", gt.shape)
    print("matrix", _conf_matrix.shape)
    print("classes", _num_classes)
    pred[predFromData == 0] = 0
    gt[predFromData == 0] = 0
    cv2.imwrite("gt.png", gt*10)
    cv2.imwrite("pred.png", pred*10)

    cv2.imwrite(f"{outputFolder}/gt/{fileName[:-4]}.png",gt*20)
    cv2.imwrite(f"{outputFolder}/pred/{fileName[:-4]}.png",pred*20)
