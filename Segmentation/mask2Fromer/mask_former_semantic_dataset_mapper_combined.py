...
class MaskFormerSemanticDatasetMapperCombined:
    ...
    def __call__(self, dataset_dict):
        ...
        combinedImage = torch.as_tensor(np.ascontiguousarray(combinedImage.transpose(2, 0, 1)))
        ...
        if self.size_divisibility > 0:
            ...
            combinedImage = F.pad(combinedImage, padding_size, value=128).contiguous()
            if sem_seg_gt is not None:
        ...
        #image = image.asarray()
        #print(type(image))
        #image.type(torch.HalfTensor)
        image=image.float()
        rgb_img = rgb_img.float()
        combinedImage = combinedImage.float()
