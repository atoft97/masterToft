...
class ValidationLoss(HookBase):
    def __init__(self, config, dataset_name):
        ...
        mapper = MaskFormerSemanticDatasetMapperStandard(config, True)
        #self._loader = detectron2.data.build_detection_train_loader(config, dataset_name)
        self._loader = build_detection_train_loader(config, mapper=mapper)
        
    def after_step(self):
        for batch in self._loader:
            loss = self.trainer.model(batch)
            print(f"validation loss: {loss}")
            self.trainer.storage.put_scalar('validation_loss', loss)
'''
...
#register_coco_panoptic_separated(name="ffi_train", metadata={}, sem_seg_root="dataset/train/panoptic_stuff_train", image_root="dataset/train/images", panoptic_root="dataset/train/panoptic_train" , panoptic_json="dataset/train/annotations/panoptic_train.json" ,instances_json="dataset/train/annotations/instances_train.json")
...
#register_coco_panoptic_separated(name="ffi_val", metadata={}, sem_seg_root="dataset/test/panoptic_stuff_test", image_root="dataset/test/images", panoptic_root="dataset/test/panoptic_test" , panoptic_json="dataset/test/annotations/panoptic_test.json" ,instances_json="dataset/test/annotations/instances_test.json")
...
#register_ffi_png_dataset(name="ffi_train_stuffonly", metadata={}, sem_seg_root="ffiRGBdataset3/train/idSegment", image_root="ffiRGBdataset3/train/image")
#register_ffi_png_dataset(name="ffi_val_stuffonly", metadata={}, sem_seg_root="ffiRGBdataset3/val/idSegment", image_root="ffiRGBdataset3/val/image")

register_ffi_png_dataset(name="ffi_train_stuffonly",   metadata={}, sem_seg_root="ffiLiDARdataset/train/idSegment", image_root="ffiLiDARdataset/train/3channelImage")
register_ffi_png_dataset(name="ffi_val_stuffonly",   metadata={}, sem_seg_root="ffiLiDARdataset/val/idSegment", image_root="ffiLiDARdataset/val/3channelImage")
...
class Trainer(DefaultTrainer):
    ...
    def build_hooks(self):
        ...
        mapper = MaskFormerSemanticDatasetMapperTest(self.cfg, True, numpyData=False)
    ...
    def build_train_loader(cls, cfg):
        ...
        '''
                # Semantic segmentation dataset mapper
        if cfg.INPUT.DATASET_MAPPER_NAME == "mask_former_semantic":
            mapper = MaskFormerSemanticDatasetMapperStandard(cfg, True)
        # Panoptic segmentation dataset mapper
        #elif cfg.INPUT.DATASET_MAPPER_NAME == "mask_former_panoptic":
        #    mapper = MaskFormerPanopticDatasetMapper(cfg, True)
...
def setup(args):
    ...
    cfg.SOLVER.BASE_LR = 0.000005#la på ein 0
    epochs = 100


    cfg.SOLVER.MAX_ITER = epochs * datasetSize
    cfg.SOLVER.STEPS = []
    cfg.TEST.EVAL_PERIOD = datasetSize
    cfg.SOLVER.CHECKPOINT_PERIOD = int((epochs * datasetSize) / 2)
    ...
    cfg.OUTPUT_DIR = "./ffiRGBLastLidar"
