...
__all__ = ["MaskFormerSemanticDatasetMapperTestCombined"]


class MaskFormerSemanticDatasetMapperTestCombined:
    """
    ...
    def __call__(self, dataset_dict):
        ...
        combinedImage = np.dstack((numpyImage, rgbImage))

        image = self.aug.get_transform(numpyImage).apply_image(numpyImage)
        image = torch.as_tensor(image.astype("float32").transpose(2, 0, 1))

        rgbImage = self.aug.get_transform(rgbImage).apply_image(rgbImage)
        rgbImage = torch.as_tensor(rgbImage.astype("float32").transpose(2, 0, 1))

        combinedImage = self.aug.get_transform(combinedImage).apply_image(combinedImage)
        combinedImage = torch.as_tensor(combinedImage.astype("float32").transpose(2, 0, 1))
        ...
        #dataset_dict["rgbImage"] = rgbImage
        #dataset_dict['image'] = image
        dataset_dict['image'] = combinedImage
        #objs obje stacked 7 layers
