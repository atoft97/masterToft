...
class MaskFormerSemanticDatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        #cv2.imwrite("testBilde.png", numpyImage[:, : ,3])
        #print("input", numpyImage.shape)
        #numpyImage = np.load("lidarDatasetNumpy/stand_still_short00001.npy")
        #print("bra2", numpyImage.shape)
            #image1 = numpyImage[:,:,0]
            #image2 = numpyImage[:,:,2:4]
            #image = np.dstack((image1, image2))
        #image = numpyImage[:,:,2:4]
        #image = copy.deepcopy(numpyImage)
        numpyImage = numpyImage.astype('float32')
        #print(numpyImage.shape)
        numpyImage = numpyImage[:,:,2:3]
        #print(numpyImage.shape)
        #print(numpyImage[:, 200:250 ,2])
        image = numpyImage
        #print("innput shape", image.shape)

        #cv2.imwrite("test1channel.png", image)
        #cv2.imwrite("test3channel.png", image)
        
        #print(fgail)
        #image = numpyImage[:,:,0:3]
        ...
        #cv2.imwrite(f"testImageEtter/{self.counter}.png", image[:,:,0:3])
        #print(image[:,200:250,2])
